...
sim.fun.prec.set.1 <- function(psi, gam.11, nu.d1, rep = 5e3, N = NA, alpha = 0.05, pi.11 = 0.5, pi.22 = 0.5)
{
  ...
  if (is.na(N)) {N = samp.size.prec.s1(gam.11, psi, nu.d1)}
  ...
  for (i in 1:rep)
  {
    ...
    if (length(m.1.NR)  != N.1.NR | length(m.1.R)  != N.1.R){next}
    ...
    w.cap <- qnorm(1 - alpha / 2) * sqrt(var.cap.Y.bar.d1)
...
{
  for (i.gam.11 in 1:length(gam.11.vec))
  {
    for (i.nu.d1 in 1:length(nu.d1.vec))
    {
      ...
      store.n[i.psi, i.gam.11, i.nu.d1] <- foo$Sample_Size
      
      #aesthetics
      print("--------------------------------")
      print(paste0("We are at psi = ", psi.vec[i.psi], ", gam.a = ", gam.11.vec[i.gam.11], ", and nu.d1 = ", nu.d1.vec[i.nu.d1],"."))
      print("--------------------------------")
    }
  }
}
...
##Select Directory
save(store.cov_prob, store.psi_hat, store.n, file = "Simulation_prec.Set.1.Rdata")

####================================================================

rm(list = ls())
load("Simulation_prec.Set.1.Rdata")
