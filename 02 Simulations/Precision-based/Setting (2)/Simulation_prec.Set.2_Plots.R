...
p1 <- df.3 |> 
  ...
  ggplot() +
  geom_ribbon(aes(x = nu.d1, ymin = 0.3325, ymax = 0.3675), fill = "gray80") +
  ...
  scale_y_continuous(breaks = c(0.32, 0.35, 0.38), limits = c(0.315, 0.385))
...
p3 <- df.5 |> 
  pivot_longer(cols = !nu.d1, names_to = "gamma", values_to = "psi.hat") |>
  ggplot() +
  geom_ribbon(aes(x = nu.d1, ymin = 0.6175, ymax = 0.6825), fill = "gray80") +
  geom_line(aes(x = nu.d1, y = psi.hat, lty = gamma)) +
  labs(x = TeX(r"($\nu_{d_1}$)"),
       y = TeX(r"($\hat{\psi}$)"),
       lty = TeX(r"($(\gamma_{11}, \gamma_{12})$  )")) +
  theme(axis.text = element_text(size = 11),
        ...
        legend.position = "none",
        plot.background = element_rect(fill = "gray100")) +
  scale_y_continuous(breaks = c(0.59, 0.65, 0.7), limits = c(0.585, 0.715))
