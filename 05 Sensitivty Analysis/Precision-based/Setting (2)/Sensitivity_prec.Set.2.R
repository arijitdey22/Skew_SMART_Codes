...
sim.fun.prec.set.2 <- function(psi, gam.11, gam.12, nu.d1, nu.samp, r1 = 1, r2 = 1,
                         rep = 1e4, N = NA, alpha = 0.05, pi.11 = 0.5, pi.22 = 0.5)
{
  ...
  if (is.na(N)){N = samp.size.prec.s2(gam.11, gam.12, psi, nu.samp)}
...
store.psi_hat <- matrix(0, ncol = length(c.nu.vec), nrow = length(nu.vec))

for (i.nu in 1:length(nu.vec)){
  for (i.c.nu in 1:length(c.nu.vec)){
    nu.true <- nu.vec[i.nu]
    nu.samp <- nu.true * c.nu.vec[i.c.nu]
    foo <- sim.fun.prec.set.2(psi, gam.11, gam.12, nu.true, nu.samp)
    
    store.psi_hat[i.nu, i.c.nu] <- foo$pis_hat
  }
}

store.psi_hat <- as.table(store.psi_hat)
colnames(store.psi_hat) <- c("c.nu = 0.75", "c.nu = 0.9", "c.nu = 1.1", "c.nu = 1.25")
rownames(store.psi_hat) <- c("nu = 1", "nu = 5")
(store.psi_hat <- round(store.psi_hat, 3))

# save(store.psi_hat, file = "Sensitivity_prec.Set.2.Rdata")
